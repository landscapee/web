...
import {  removeToken, removeUserInfo } from '@lib/auth';
...
instance.interceptors.request.use(config => {
    if((store.getters.token===null || !store.getters.token || store.getters.token==='') && config.url!="/api/sso/login/login"){
...
instance.interceptors.response.use(response => {
    ...
    //     });
    // }
    if (response.code == 415 || (response.data && response.data.code == 401)) {
        removeToken();
        removeUserInfo();
        router.push({path:"/"});
    }
