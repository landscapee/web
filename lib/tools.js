    ...
    let a = Array.from(document.getElementsByClassName('el-date-editor'))
     a.map((k, l) => {
         k.addEventListener('mouseover', (e) => {
             if (e.target.value) {
                k.getElementsByClassName(ele||'el-icon-date')[0].style = "display:none"
            }
        } )
        k.addEventListener('mouseout', (e) => {
            k.getElementsByClassName(ele||'el-icon-date')[0].style = {display: 'black'}
        } )
...
export const inputLength=(t)=>{
    ...
    textarea.map((k,l)=>{
        k.addEventListener('input', (e) => {
            if (e.target.value&&e.target.value.toString().length>225) {
                e.target.value=e.target.value.slice(0,225)
                // k.style = "border-color:red"
                t.$message.warning('最多只能输入225个字符')
