...
export const bindInputFn=(that)=>{
    ...
    for(let i=0;i<arr.length;i++){
        ...
        arr[i].addEventListener('focus',($event)=>{inputTypeFocus( arr[i],$event,that)})
    }
}
export const inputTypeFocus=(k,e,that)=>{
    let type=k.getAttribute('inputtype')

    if(type=='num'&&e.target.value=='NA'){
        e.target.value=null
    }
}
export const numtypeC=(type,e,that)=>{
    // let type=k.getAttribute('inputtype')
    ...
    if(type==='num'){
        ...
        if(!blo){
            debounce(()=>{
                that.$message.warning('只能输入数字默认为NA')
            },150)()
        }
    }
}
export const numtypeB=(type,e,that)=>{
    // let type=k.getAttribute('inputtype')
    ...
    if(type==='num'){
        ...
        if(value.split('-').length>1&&value.split('-')[1]===''){
            e.target.value= 'NA'
            that.$message.warning('请输入正确的数字')
        }

    }
}
function inputTypeBlur(ele,e,that) {
    let type=ele.getAttribute('inputtype')
    if(type=='num'&&e.target.value===''){
        e.target.value="NA"
        return false
    }
    numtypeB(type,e,that)
}
function inputTypeC(ele,e,that) {
    let type=ele.getAttribute('inputtype')
    if(e.target.value===''){
        return false
    }
