    ...
    export default {
        ...
        methods: {
            ...
            getAllUserByOrgId(node ){
                let params={}
                let url='sys/role/geAllRoleByRoleTypeIdByPage'
                 if(node.type=='sys'){
                     url='sys/role/getAllRoleBySysCode'
                     params.code=node.code
				}else{
                    params.roleTypeId=node.id
				}
                return request({
            ...
            getTree() {
                 request({
                    url: 'sys/subsystem/getSubsystemByCode',
                    method: 'get',
                    params:{code:'maintenance'},
                }).then((d1) => {
                    if(d1.responseCode==1000){
                        request({
                            url: 'sys/role/getAllRoleTypeBySysCode',
                            method: 'get',
                            params:{code:'maintenance'},
                        }).then((d) => {
                            this.data=[
