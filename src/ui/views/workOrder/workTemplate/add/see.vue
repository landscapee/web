		...
		<div class="order">
			<div class="head" style="width: 100%">
				...
				<table class="nomTable nomTable0" style="width: 100%">
					<tr class="trimg" v-if="get(form.typeVO,'airlineCompany')||form.photoPath">
						<td>
							<div><img :src="form.photoPath" alt=""></div>
						</td>
			...
			<div class="info" v-if="getbaseItemVOList&&getbaseItemVOList.length>0">
				<div class="lastRow" v-for="(opt) in Math.ceil(getbaseItemVOList.length/3)" :key="opt">
					<el-row>
						<el-col class="colCenter" :span="8" v-for="(opt1,index) in getArr(opt)" :key="index"
								v-if="opt1.enable||!opt1.type">
							<div>{{opt1.nameCn}}</div>
							<div>{{opt1.nameEn}}</div>
						</el-col>
					</el-row>
					<el-row>
						<el-col :span="8" v-for="(opt1,index) in getArr(opt)" :key="index"
								v-if="opt1.enable||!opt1.type">
							...
							<div v-else-if="opt1.type==5" style="width:100%">
								<div>
									<div v-for="(k,l) in opt1.value.split(';')" :key="l"
										 style="width:100%;word-wrap: break-word;margin-top: 5px">
										<input type="checkbox" :name="opt1.placeholder.split(';')[l]"
											   class="Wtui-checkbox"></input>{{k}}
			...
			<div class="item">
				<table class="one" border="0" width="100%">
					<tr height="45px">
						...
						<td v-if="form.labelVO"
							:width="get(form.labelVO,'contentLayout')=='C4（四列）'?'50%':get(form.labelVO,'contentLayout')=='C3（三列）'?'66%':'83%'">
							<div>{{get(form.labelVO,'contentLabel')}}</div>
							<div>{{get(form.labelVO,'contentLabelEnglish')}}</div>
						</td>
						<td width="17%"
							v-if="get(form.labelVO,'contentLayout')=='C3（三列）'||get(form.labelVO,'contentLayout')=='C4（四列）'">
					...
					<tr v-if="contentVOList&&contentVOList.length" v-for="(opt,index) in contentVOList" :key="index"
						:class="(opt.itemType==1||opt.itemType==3)&&opt.contentDetails&&opt.contentDetails.length?'none':''">
						<template v-if="(opt.itemType==1||opt.itemType==3)">
							...
							<template v-else>
									...
									<div style="display: flex;justify-content: space-between;padding: auto 30px">
										<div style="margin-right: 10px" class="logo">
											...
											<div v-if="opt.cycle">
												...
												<icon-svg iconClass="quanUncheck"
														  style="height:20px;width:30px"></icon-svg>
												<div v-if="show" style="width:60px;word-wrap: break-word">
								...
								<td width="17%"
									v-if="get(form.labelVO,'contentLayout')=='C3（三列）'||get(form.labelVO,'contentLayout')=='C4（四列）'"></td>
								<td width="17%" v-if="get(form.labelVO,'contentLayout')=='C4（四列）'">
						...
						<template v-else>
							...
							<td class="contenttable" v-html="opt.content"
								:width="get(form.labelVO,'contentLayout')=='C4（四列）'?'50%':get(form.labelVO,'contentLayout')=='C3（三列）'?'66%':'83%'"
							...
							<td width="17%"
								v-if="get(form.labelVO,'contentLayout')=='C3（三列）'||get(form.labelVO,'contentLayout')=='C4（四列）'"
								:class="opt.workerLabel?'':'duijiao'">
								<div v-if="opt.workerLabel">
									<div>
										{{opt[get(form.labelVO,'contentLayout')=='C3（三列）'?'c3Remark':'c4WorkerRemark']}}
									</div>
							...
							<td width="17%" v-if="get(form.labelVO,'contentLayout')=='C4（四列）'"
								:class="opt.commanderLabel?'':'duijiao'">
