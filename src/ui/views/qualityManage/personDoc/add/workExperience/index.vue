...
export default {
   ...
   created() {
    ...
    methods: {
        ...
        addOrEditOrInfo(tag){
             if(tag=='add'){
                this.$router.push({path:'/workExperienceAdd',query:{type:'add',rId:this.id+','+this.type+','+this.userId}});
            }else if(tag == 'edit' || tag == 'info'){
                if(this.selectId==null){
                    this.$message.error('请先选中一行数据');
                }else{
                     this.$router.push({path:'/workExperienceAdd',query:{type:tag,rId:this.id+','+this.type+','+this.userId,id:this.selectId}});
                }
            }
        },
        delData(){
