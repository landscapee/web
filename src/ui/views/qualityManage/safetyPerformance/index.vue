<template>
	<div>
		<router-view v-if="this.$router.history.current.path == '/safetyPerformanceAdd'"
					 :key="$route.path"></router-view>
		<router-view v-else-if="this.$router.history.current.path == '/safetyPerformanceDetailsAdd'"
					 :key="$route.path"></router-view>
		<router-view v-else-if="this.$router.history.current.path == '/safetyPerformanceYear'"
					 :key="$route.path"></router-view>
		<div v-else-if="this.$router.history.current.path == '/safetyPerformance'" class="businessData">
				...
				<div class="top-toolbar">
					...
					<div class="headDiv headDiv2">
						...
						<div class="right-toolbar">
							<div @click="rightyear('add')">
								<icon style="width: 0!important;" iconClass=""></icon>
								部门年度安全绩效
							</div>
			...
			<div class="main-content">
				<SearchTable class="left-main-table" :data="tableLeftData" :tableConfig="businessTableConfig"
							 refTag="left-table" ref="left-table"
					...
					<el-table-column slot="option" label="操作" align="center" :width="80">
						<template slot-scope="{ row }">
                                <span style="padding:2px  " class="rowSvg" @click="row.copy?'':copyDetails(row)"
									  :class="row.copy?'G_isDisabled':''">
	...
	export default {
		...
		methods: {
			...
			handleScroll($event) {
				...
				// 获取距离顶部的距离
				var scrollTop = Math.ceil(bady.scrollTop);
				// 获取可视区的高度
				...
				console.log(bady.offsetHeight,windowHeight,scrollTop,);
				...
				if (tag == 'add') {
					if (this.leftSelectId == null) {
						...
						this.$router.push({
							...
							query: {type: 'add', securityMeritsId: this.leftSelectId}
						});
					}
				} else if (tag == 'edit' || tag == 'info') {
					if (this.rightSelectId == null) {
						...
						this.$router.push({
							path: '/safetyPerformanceDetailsAdd',
							query: {type: tag, securityMeritsId: this.leftSelectId, id: this.rightSelectId}
						});
					}
				}
			},
